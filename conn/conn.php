...
function ADDBB($nome,$email,$cel,$cpf,$bbb,$numero) {
    ...
    function RELATORIO() {
        ...
        $conexao = MyCONN(); 
          $query =" select count(id) as INSCRITOS ,lugar as local from form_exped_lib where lugar = 'SOU'
                     UNION
                     select count(id) as ALTAMIRA ,lugar as local from form_exped_lib where lugar = 'ALT' ";
         $listas = mysqli_query($conexao, $query);
         return $listas;  
